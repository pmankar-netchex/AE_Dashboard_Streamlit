...
S6_COL_AM = SOQLEntry(
    col_id="S6-COL-AM",
    display_name="Self-Gen Bookings $ (Period)",
    section="Self-Gen Pipeline Creation",
    description="Closed-won revenue (split-credited) from Net New opportunities self-generated by the AE (the AE is both creator and split-owner), with a Close Date in the selected period.",
    aggregation="SUM(SplitAmount)",
    time_filter=True,
    template="""
SELECT SUM(SplitAmount) total
...
S6_COL_AN = SOQLEntry(
    col_id="S6-COL-AN",
    display_name="SDR Bookings $ (Period)",
    section="SDR Activity",
    description="Closed-won revenue (split-credited) from Net New opportunities sourced by this AE's SDR(s), with a Close Date in the selected period.",
    aggregation="SUM(SplitAmount)",
    time_filter=True,
    template="""
SELECT SUM(SplitAmount) total
...
S6_COL_AO = SOQLEntry(
    col_id="S6-COL-AO",
    display_name="CP Bookings $ (Period)",
    section="Channel Partners",
    description="Closed-won revenue (split-credited) from Net New opportunities sourced from channel partners, with a Close Date in the selected period.",
    aggregation="SUM(SplitAmount)",
    time_filter=True,
    template="""
SELECT SUM(SplitAmount) total
...
S6_COL_AP = SOQLEntry(
    col_id="S6-COL-AP",
    display_name="Marketing Bookings $ (Period)",
    section="Marketing",
    description="Closed-won revenue (Revenue split) from Net New opportunities attributed to the Marketing source category, with a Close Date in the selected period.",
    aggregation="SUM(SplitAmount)",
    time_filter=True,
    template="""
SELECT SUM(SplitAmount) total
...
ALL_COLUMNS: list[SOQLEntry] = [
    ...
    S6_COL_AH, S6_COL_AN, S6_COL_AG, S3_COL_T, S3_COL_U, S3_COL_V, S3_COL_W,
    # Section 4 — CP: Pipeline $, Bookings $, Opps, Emails, Calls, Mtgs Scheduled, Mtgs Held
